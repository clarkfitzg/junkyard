...
a = cumsum(a)
coefpd = (a['X3', 'Mean Sq'] - a['X4', 'Mean Sq']) / a['X4', 'Mean Sq']
coefpd 
```
...
We perform the correlation transformation:
...
cor.transform = function(X, scaleinfo, inverse=FALSE){
    ...
    if (inverse){
        Xnew = data.frame(X * s$sd / s$multiplier + s$mean)
    }
    else{
        Xnew = data.frame(s$multiplier * (X - s$mean) / s$sd)
    }
    return(Xnew)
}

scl = getscale(property)
property.t2 = cor.transform(property, scl)

# testing correctness
property.t = scale(property) / sqrt(nrow(property) - 1)
pback = cor.transform(property.t2, scl, inverse=TRUE)
```
...
```{r, echo=FALSE}
unstandardize = function(x, n=n, means, sds){
    # The inverse of a correlation transformation
    #   x   : vectors to be transformed back
    #   n   : as used in the correlation transformation
    #   means   : vector of column means of original data
    #   sds     : vector of column standard deviations of original data
